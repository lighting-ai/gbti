...
function applyOptionalSiteOriginMeta() {
  ...
  document.querySelector('meta[property="og:image"]')?.setAttribute('content', imageAbs)
  document.querySelector('meta[name="twitter:image"]')?.setAttribute('content', imageAbs)
}
...
function initHeroSpotlight(config, typeByCode) {
  ...
  function setHeroSpotlight(index, restartAuto = true) {
    ...
    if (spotlightState.transitionTimer) clearTimeout(spotlightState.transitionTimer)
...
async function init() {
  ...
  const quiz = createQuiz(questions, config, { dimensionMeta: dimensions.meta })

  function runResult() {
    const answers = quiz.getAnswers()
    const rawScores = calcDimensionScores(answers, questions.main, dimensions.order)
    const levels = scoresToLevels(rawScores, config.scoring.levelThresholds)
    const result = determineGbResult(answers, levels, dimensions.order, standardTypes, config)
